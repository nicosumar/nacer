...
class DatoReportableAsociado < ActiveRecord::Base
  ...
  def dato_reportable_necesario_presente?
    drr = (DatoReportableRequerido.where(:prestacion_id => prestacion_brindada.prestacion_id, :dato_reportable_id => dato_reportable_id) || []).first
    if drr.nil? || !drr.necesario || !eval("valor_" + drr.dato_reportable.tipo_ruby + ".blank?")
      true
    else
      errors.add(("valor_" + drr.dato_reportable.tipo_ruby).to_sym, "El valor del campo \"" + (drr.dato_reportable.nombre_de_grupo ? drr.dato_reportable.nombre_de_grupo + " " + drr.dato_reportable.nombre.mb_chars.downcase.to_s : drr.dato_reportable.nombre) + "\" no puede estar en blanco")
      false
    end
  end
  ...
  def hay_advertencias?
    ...
    if drr.obligatorio && eval("valor_" + drr.dato_reportable.tipo_ruby + ".blank?")
      if @advertencias.has_key?(("valor_" + drr.dato_reportable.tipo_ruby).to_sym)
        @advertencias[("valor_" + drr.dato_reportable.tipo_ruby).to_sym] << "El valor del campo \"" + (drr.dato_reportable.nombre_de_grupo ? drr.dato_reportable.nombre_de_grupo + " " : "") + drr.dato_reportable.nombre.mb_chars.downcase.to_s + "\" no puede estar en blanco"
      else
        @advertencias.merge!(("valor_" + drr.dato_reportable.tipo_ruby).to_sym => ["El valor del campo \"" + (drr.dato_reportable.nombre_de_grupo ? drr.dato_reportable.nombre_de_grupo + " " + drr.dato_reportable.nombre.mb_chars.downcase.to_s : drr.dato_reportable.nombre) + "\" no puede estar en blanco"])
      end
