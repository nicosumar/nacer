...
class RendicionesController < ApplicationController
  ...
  def edit
    @rendicion = Rendicion.find(params[:id])
    if @rendicion.estado_de_rendicion_id == 3
      redirect_to @rendicion, notice: 'La rendicon de encuentra cerrada y no puede ser modificada.' 
    end
  end
  ...
  def imprimir
    ...
    # Armar el reporte
    reporte = ODFReport::Report.new("lib/assets/informe_rendicion2.odt") do |r|
