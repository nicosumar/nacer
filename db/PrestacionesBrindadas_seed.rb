    ...
    CREATE OR REPLACE FUNCTION prestaciones_brindadas_fts_trigger() RETURNS trigger AS $$
      ...
      BEGIN
        ...
        ELSIF (TG_OP = 'UPDATE' AND new_indexable AND old_indexable) THEN
          ...
          IF novedad IS NOT NULL THEN
            SELECT LOWER(nombre) INTO clase_de_documento FROM clases_de_documentos WHERE id = afiliado.clase_de_documento_id;
            SELECT codigo INTO tipo_de_documento FROM tipos_de_documentos WHERE id = NEW.tipo_de_documento_id;
          ELSE
            SELECT LOWER(nombre) INTO clase_de_documento FROM clases_de_documentos WHERE id = novedad.clase_de_documento_id;
          ...
          SELECT nombre INTO prestacion FROM prestaciones WHERE id = NEW.prestacion_id;
          SELECT nombre INTO diagnostico FROM diagnosticos WHERE id = NEW.diagnostico_id;
          UPDATE busquedas_locales SET
            ...
            texto =
              ...
              ', clave ''' ||
              COALESCE(clave_de_beneficiario, '') ||
              ''', documento ' ||
              ...
              COALESCE(to_char(NEW.fecha_de_la_prestación, 'DD/MM/YYYY'), '') ||
               || '.',
            vector_fts =
