...
class ModifyEstadosDeLasPrestaciones < ActiveRecord::Migration
  ...
  def change
    ...
    EstadoDeLaPrestacion.find(2).update_attributes({:nombre => 'Registrada, con advertencias', :codigo => 'F', :pendiente => true, :indexable => true})
    ...
    EstadoDeLaPrestacion.create({:nombre => 'Aprobada para liquidación', :codigo => 'L', :pendiente => false, :indexable => false})
    EstadoDeLaPrestacion.create({:nombre => 'Rechazada por la UGSP', :codigo => 'Z', :pendiente => false, :indexable => false})
    EstadoDeLaPrestacion.create({:nombre => 'Devuelta por la UGSP para refacturar', :codigo => 'V', :pendiente => true, :indexable => true})
