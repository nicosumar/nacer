      ...
      $BODY$
        ...
        BEGIN
          ...
          IF (TG_OP = 'DELETE') THEN
            ...
            EXECUTE '
              DELETE FROM ' || TG_TABLE_SCHEMA || '.busquedas_locales WHERE modelo_type = ''PrestacionBrindada'' AND modelo_id = ' || OLD.id || ';
            ';
            RETURN OLD;
          ELSIF (TG_OP = 'UPDATE' AND NOT new_indexable AND old_indexable) THEN
            ...
            EXECUTE '
              DELETE FROM ' || TG_TABLE_SCHEMA || '.busquedas_locales WHERE modelo_type = ''PrestacionBrindada'' AND modelo_id = ' || NEW.id || ';
            ';
            RETURN NEW;
          ELSIF (TG_OP = 'UPDATE' AND new_indexable AND old_indexable) THEN
            ...
            EXECUTE '
              SELECT *
                FROM ' || TG_TABLE_SCHEMA || '.novedades_de_los_afiliados novedades
                LEFT JOIN estados_de_las_novedades estados
                  ON (estados.id = novedades.estado_de_la_novedad_id)
                WHERE
                  clave_de_beneficiario = ''' || COALESCE(NEW.clave_de_beneficiario, '') || ''' AND estados.pendiente
            ' INTO novedad;
            ...
            EXECUTE '
              UPDATE ' || TG_TABLE_SCHEMA || '.busquedas_locales SET
                titulo =
                ...
                texto =
                  ...
                  '.'',
                vector_fts = ''' ||
                  replace((setweight(to_tsvector('public.indices_fts', 'Código: '), 'D') ||
          ...
          ELSIF (new_indexable AND (TG_OP = 'INSERT' OR TG_OP = 'UPDATE' AND NOT old_indexable)) THEN
            ...
            EXECUTE '
              INSERT INTO ' || TG_TABLE_SCHEMA || '.busquedas_locales (modelo_type, modelo_id, titulo, texto, vector_fts) VALUES (
                ''PrestacionBrindada'', ' ||
                NEW.id || ',
                ''Prestación pendiente: ' ||
                COALESCE(apellido || ', ', '') ||
