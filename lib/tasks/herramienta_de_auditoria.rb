  ...
  def self.auditar_tabla(arg_nombre_de_tabla)
    ...
    begin
      ActiveRecord::Base.connection.execute ""+
        ...
        "            INSERT INTO #{tabla_auditoria} SELECT 'DEL', now(), OLD.*;\n"+
        "            RETURN OLD;\n"+
        "        ELSIF (TG_OP = 'UPDATE') THEN\n"+
        "            INSERT INTO #{tabla_auditoria} SELECT 'NEW', now(), NEW.*;\n"+
        "            INSERT INTO #{tabla_auditoria} SELECT 'OLD', now(), OLD.*;\n"+
        "            RETURN NEW;\n"+
        "        ELSIF (TG_OP = 'INSERT') THEN\n"+
        "            INSERT INTO #{tabla_auditoria} SELECT 'INS', now(), NEW.*;\n"+
        "            RETURN NEW;\n"+
        "        END IF;\n"+
        "        RETURN NULL; \n"+
        "    END;\n"+
        "$#{tabla_auditoria}$ LANGUAGE plpgsql;\n"+
        "CREATE TRIGGER #{tabla_auditoria}\n"+
