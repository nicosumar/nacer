    ...
    ActiveRecord::Base.transaction do
      ...
      liquidacion.each do |l|
        ...
        args_arr_efectores.each do |e|
          ...
          vigencia_perstaciones = l.periodo.dias_de_prestacion
          ...
          formula = "Formula_#{l.concepto_de_facturacion.formula.id}"
          ...
          pliquidadas = l.prestaciones_liquidadas.where(efector_id: e)
          ...
          efector = Efector.find(e)
          cuasifactura = LiquidacionSumarCuasifactura.create!( liquidacion_sumar: l, 
                                                              efector: efector, 
                                                              monto_total: total_cuasifactura, 
                                                              concepto_de_facturacion: l.concepto_de_facturacion)
          
          # 2) Obtengo el numero de cuasifactura si corresponde para este tipo de documento
          documento_generable = l.concepto_de_facturacion.documentos_generables_por_conceptos.where(documento_generable_id: 1).first
          ...
          # 5 ) Genero los consolidados para quienes correspondan.
            ...
            c = ConsolidadoSumar.where(efector_id: administrador.id, liquidacion_sumar_id: l.id)
            ...
            when 0 #No existe un consolidado para este efector administrador y esta liquidación
              ...
              if administrador.cuasifacturas.where(liquidacion_sumar_id: l.id).size > 0
                total_cuasifactura_administrador += administrador.cuasifacturas.where(liquidacion_sumar_id: l.id).first.monto_total
              end

              total_cuasifactura_administrados = 0
              administrador.efectores_administrados.each do |ea|
                if ea.cuasifacturas.where(liquidacion_sumar_id: l.id).size > 0
                  total_cuasifactura_administrados += ea.cuasifacturas.where(liquidacion_sumar_id: l.id).first.monto_total
                end
              end
              ...
              if total_consolidado > 0
                ...
                administrador.efectores_administrados.each do |ea|
                  ...
                  if ea.cuasifacturas.where(liquidacion_sumar_id: l.id).size > 0
                    monto = ea.cuasifacturas.where(liquidacion_sumar_id: l.id).first.monto_total
                  else
                    monto = 0
                  end
